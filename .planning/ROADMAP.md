...
Latest checkpoint: Consolidation summarization now materializes a one-level gist projection. The summarizer appends `consolidation-summary` evidence carrying `abstractive_gist` metadata and derived capability tags, then writes `summary-derived-gist` source-to-summary relations so gist provenance participates in export, audit, and erasure paths. This narrows the gist/RAPTOR gap; multi-level RAPTOR clustering, summary refresh scheduling, and production forgetting-policy validation remain open.
...
Latest checkpoint: One-level gist refresh now has explicit retirement semantics. Summary materialization records a source fingerprint and active status; rerunning the summarizer over the same source set creates a new active summary, retires the previous one with supersession metadata, and local/Postgres retrieval filter retired summary evidence before ranking. Forget now walks transitive derived-evidence provenance through source and summary CID metadata, so raw-source erasure invalidates derived gist evidence and expires summary relations. This closes the stale-summary/source-forget safety gap while bounded refresh scheduling and multi-level RAPTOR trees remain open.

Latest checkpoint: Projection recompute now performs bounded summary-refresh scheduling. Changed source evidence expands to derived one-level gist evidence through summary metadata and `summary-derived-gist` relations, affected summary relations are reported in the projection set, and the recompute job queues consolidation refreshes for surviving non-summary source evidence only. This gives gist refresh an async affected-subgraph trigger while multi-level RAPTOR clustering and production differential recompute still remain open.
