...
Checkpoint: CLI-first file ingestion now supports signed-provenance JSON and a `c2patool`-style verifier adapter; quarantined evidence is hidden from default retrieval. Trust-tier semantics now follow the blueprint scale: `0` direct-user/highest trust through `5` untrusted external. Ingest classification now tags actor/source trust, untrusted imperative content, and simple PII. Plan 06-04 remains open for production trust-policy mapping, derived multimodal extraction/indexing, and queue-backed consolidation.
