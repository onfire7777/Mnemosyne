...
Latest checkpoint: CLI `deployment-soak` now runs allowlisted deployment preflight manifests through shell-free subprocesses for `provider-check`, `idp-jwks-live-check`, `tls-cert-check`, `mcp-http-soak`, `mcp-sse-soak`, and `worker-run`. It parses child JSON reports, omits command arguments, reports required/optional failures, and fails closed on required check failures, timeouts, non-JSON output, or disallowed commands. Tests cover a real local hosted HTTP plus legacy SSE manifest without leaking env-provided tokens and disallowed-command rejection.
