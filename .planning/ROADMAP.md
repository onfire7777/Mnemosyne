...
Latest checkpoint: Shared Local/Postgres branch coverage now verifies explicit source-branch inheritance. Creating a child branch from a non-`main` source branch copies both main-inherited and source-only evidence into the child, keeps source-only evidence isolated from `main`, and exports the child branch `from_branch` metadata for auditability.
