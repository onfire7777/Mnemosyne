...
Checkpoint: CLI-first file ingestion now supports signed-provenance JSON and a `c2patool`-style verifier adapter; quarantined evidence is hidden from default retrieval. Trust-tier semantics now follow the blueprint scale: `0` direct-user/highest trust through `5` untrusted external. Ingest classification now tags actor/source trust, untrusted imperative content, and simple PII. Ingestion now enqueues a persisted local consolidation job, CLI commands can inspect the queue or run one worker cycle, and the worker can extract a simple direct-user fact candidate through the promotion gate while refusing data-only untrusted facts. Plan 06-04 remains open for production trust-policy mapping and derived multimodal extraction/indexing; Plan 06-05 remains open for durable production PGMQ/Redis-style worker orchestration across consolidation, calibration, lifecycle, eval, and observability jobs.
