...
Latest checkpoint: `mneme-mcp --self-test` now provides a secret-redacted local deployment preflight for MCP initialize/list/schema/auth/session/read-only-call behavior. This is not hosted parity; HTTP/SSE or equivalent hosted transport validation, TLS/client identity, production IdP/session issuance, and production stateless soak testing remain open. CLI `provider-check` now also rejects command KMS providers that claim `shred_key` success while still reporting or returning the post-shred key.
