...
- Add production auth/session identity, deployment-grade authorization integration, real KMS deployment validation behind the command key-provider boundary, production C2PA certificate-chain/root validation, native image/audio embedding providers, production extractor deployment validation, cross-region residency controls, worker supervision/deployment operations, and deployment observability.
