...
Latest checkpoint: Official MCP SDK StreamableHTTP runtime/CLI soak coverage is implemented, live CLI/Postgres HTTP retrieval-provider smoke proves provider-generated 1024-dim pgvector evidence storage and HTTP reranker ordering, and command-backed raw-media embeddings are now wired into actual ingestion/retrieval. Raw externalized image/audio/video evidence can carry a stored media vector before derived text exists; LocalMemoryEngine and PostgresEngine use that vector for dense retrieval, and live CLI/Postgres verifies command media embedding storage plus vector retrieval. Local verification collects 313 tests; the compose Postgres/shared suite remains green with `MNEMOSYNE_POSTGRES_DSN=postgresql://mnemosyne:...@127.0.0.1:54329/mnemosyne` across 66 live/shared tests. Exact parity still remains open for real provider deployment validation, real IdP/JWKS deployment validation against live credentials, production certificate lifecycle, hosted SSE/operator validation, and production stateless soak testing.
