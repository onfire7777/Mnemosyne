...
Latest checkpoint: Parametric evaluation and rollback now prefer persisted protected regression cases from runtime state, emit stable protected-suite snapshots, persist rollback suite metadata, and pass protected-suite evidence to command-backed rollback providers through CLI/MCP. Focused parametric CLI/MCP tests, the full affected runtime/CLI files, and the full local pytest suite pass. Local verification still collects 313 tests; the compose Postgres/shared suite remains green with `MNEMOSYNE_POSTGRES_DSN=postgresql://mnemosyne:...@127.0.0.1:54329/mnemosyne` across 66 live/shared tests. Exact parity still remains open for real provider deployment validation, real IdP/JWKS deployment validation against live credentials, production certificate lifecycle, hosted SSE/operator validation, production stateless soak testing, and deployed trainer protected-suite/rollback validation.
