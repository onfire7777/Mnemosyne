...
- [Privacy]: Local and Postgres forget paths now accept `tombstone_recompute` or `hard_delete_legal`; CLI exposes `--erasure-mode`; encrypted local object storage supports AES-GCM per-object keys and legal hard-delete crypto-shreds the object key after successful forget.
...
- Add production auth/session identity, deployment-grade authorization integration, production KMS-backed key management, production C2PA trust validation, native image/audio embedding providers, production extractor deployment validation, data residency enforcement, and deployment observability.
