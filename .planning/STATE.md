...
- [Observability]: CLI `ops-report` now exports dashboard-ready counts, queue state, learning diversity, proxy-vs-true gap, and tripwire status for a tenant.
...
- Add production auth/session identity, deployment-grade authorization integration, production KMS-backed key management, production C2PA trust validation, native image/audio embedding providers, production extractor deployment validation, cross-region residency controls, and deployment observability.
