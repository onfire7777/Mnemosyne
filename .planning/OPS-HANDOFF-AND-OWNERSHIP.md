...
| **E** | Observability | Live dashboards, alert routing, SLO monitors; `ops-report` + `ops-dashboard-check` live wiring | Deploy steps, rollback, secret provisioning | hosted dashboard URL plus retained `ops-dashboard-bundle.json`; package input only if pre-existing external custody evidence |
...
| 8 | Observability dashboards | `ops-dashboard-check` (+ `ops-report`) | Hosted production dashboard URL plus retained `ops-dashboard-bundle.json`; package mode only for pre-existing external input evidence, not `ops-report --dashboard-package-dir` generation during capture | E (primary), G | hosted-dashboard ops evidence in bundle; `release-audit` ok |
...
**Universal acceptance pattern (every row):** operator runs the gate against real infra → evidence redacted → wrapper includes it in `deployment-soak --evidence-dir` (production scope + operator attestation) → `release-audit --evidence-manifest "$OUT_ROOT/evidence/manifest.json" --require-production-validated --require-provider-forbid-local` passes with that command's output shape present and `findings` empty → `production-evidence-verify` passes offline against the retained `summary.json` bundle fingerprint plus `preflight.json`, `redaction-scan.json`, `bundle-manifest.json`, `source-soak-manifest.json`, `operator-soak-manifest.json`, and `input-artifacts/` custody. Successful captures include `summary.json.offline_verify` with the exact argv-style offline verifier command reviewers should run, including the wrapper-selected interpreter as argv[0].
