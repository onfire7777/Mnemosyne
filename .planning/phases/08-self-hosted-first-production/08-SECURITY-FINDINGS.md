...
- `ProvenanceTrustPolicy` now defaults to fail-closed issuer matching for C2PA verifier reports.
  Remaining work: production C2PA roots/issuers and rotation evidence.
...
8. **Provenance fail-closed:** `require_trusted_issuer=True` (+ `require_trusted_root` when roots set); self-signed/untrusted/digest-mismatch → data-only, never raises trust; `provenance-trust-check` fails on empty/dev roots. **Code status:** default C2PA trust policy now rejects unconfigured or mismatched signers; root-only trust must be explicitly opted in with `require_trusted_issuer=false` plus root enforcement. Live C2PA root/issuer rotation evidence remains required.
...
16. **Tamper-evident audit log:** hash-chained append-only `audit_log` (prev_hash/row_hash keyed by a Vault-held HMAC, BEFORE UPDATE/DELETE trigger raises, INSERT-only grant, break-glass-only mutate); `audit-verify` run in CI against the live DB; pgaudit + an off-box append-only (Loki/WORM) second copy the app role cannot rewrite; log every write AND every auth decision (actor/source/tier/diff/reason, hashed PII). **Code status:** source now has the append-only Postgres trigger, INSERT-only runtime role grants, and shared engine auth-decision audit events from `MemoryTools._authorize`; Vault-HMAC hashes, pgaudit/WORM export, live `audit-verify`, and retained operator evidence remain open.
