...
- **07-05 — P5: retire the toggles + finalize.** Remove the legacy `service.enabled`, `SpecialistBudget.shadow_only`, and controller `budget.shadow_only` controls once Standing fully subsumes them (owner checkpoint before removal). Extend contradiction/erasure **cascade to Standing + self-derivations (H8)**; full **observability/reversibility (H12)**. Final 14-indicator scorecard + a unified-substrate audit note. Result: one cohesive, always-on substrate with explicit advisory promotion left only behind separate, CID-validated gates.
</specifics>
