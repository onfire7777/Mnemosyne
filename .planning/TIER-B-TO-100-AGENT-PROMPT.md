...
- No secrets in manifests/args/docs (wrapper rejects `--access-token`/`--api-token`/`--github-token`/`--session-secret`/`--password` and fails closed on secret material). Absolute, non-symlinked, external executables for every provider `command` + `MNEMOSYNE_PROD_C2PA_TOOL`; provider commands must not include path-like arguments after `argv[0]` because those targets are not retained under `tool-artifacts/`. Hosted dashboard/probe URLs and HTTP embedding/reranker provider URLs must flow through the shared fail-closed `network_safety` validator/opener, not raw `urlopen`. Keep expected fingerprints out-of-band. Report only real measured latency/recall.
