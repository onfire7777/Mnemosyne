...
- Externalized multimodal evidence now indexes metadata-derived `derived_text`, OCR text, transcripts, captions, alt text, and descriptions while keeping raw bytes in object storage; local, CLI, and live Postgres tests verify retrieval over the derived text.
...
2. Wire production embedding, cross-encoder, and native image/audio extraction providers behind the existing adapter boundaries.
