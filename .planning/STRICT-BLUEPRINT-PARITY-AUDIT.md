...
- Verification currently passes with `.venv/bin/python -m compileall -q src tests` and `.venv/bin/python -m pytest -q` returning 78 passing tests plus 3 skipped live-DB tests.
...
| Consolidation role pipeline | Partial | Ingest now queues ordered persisted local consolidation pass payloads, and the CLI can inspect pending jobs or run one worker cycle. Remaining work includes production PGMQ/Redis-style idle/server worker orchestration, extraction/summarization roles, entity resolution, incremental recompute, and protected-suite gate persistence. |
