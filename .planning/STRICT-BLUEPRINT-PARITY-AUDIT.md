...
- Verification currently passes with `.venv/bin/python -m compileall -q src tests` and `.venv/bin/python -m pytest -q` returning 89 passing tests plus 7 skipped live-DB tests.
...
- Fail-closed capability enforcement added in `MemoryTools` for preference writes, hard-instruction profile writes, destructive forget operations, fact assertions, relations, proposal branches, supersessions, and corrections. CLI and facade tests verify denied low-trust writes and allowed trusted writes; CLI `assert` and `relation` now route through the facade instead of bypassing authorization.
...
| Tenant isolation and auth | Partial | Capability checks now protect preference, hard-instruction profile, destructive forget, fact, relation, proposal, supersession, and correction writes; Postgres tenant RLS is implemented and live-smoked; MCP `tools/call` has opt-in token enforcement. Trust-tier direction now matches the blueprint scale. Remaining work: branch/promotion authorization hardening, production identity/session handling, and deployment-grade auth integration. |
...
4. Harden branch/promotion authorization and add production identity/session handling before making any production multi-tenant claim.
