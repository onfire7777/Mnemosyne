...
- Verification currently passes with `.venv/bin/python -m compileall -q src tests` and `.venv/bin/python -m pytest -q` returning 58 passing tests plus 1 skipped live-DB test.
- Live Docker/Postgres verification passes with `MNEMOSYNE_POSTGRES_DSN=postgresql://... .venv/bin/python -m pytest -q tests/test_postgres_engine_live.py` returning 1 passing test against the compose database and verifying SQL FTS, pgvector assertion search, dense evidence fallback, recursive graph/PPR, branch/discard, forget, and export.
