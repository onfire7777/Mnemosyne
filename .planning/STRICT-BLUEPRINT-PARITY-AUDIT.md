...
- Verification currently passes with `.venv/bin/python -m compileall -q src tests` and `.venv/bin/python -m pytest -q` returning 74 passing tests plus 3 skipped live-DB tests.
...
- Ingest classification now identifies actor/source trust, applies capability/taint tags, detects email/phone/SSN-style PII, raises sensitivity/access policy metadata, and marks untrusted imperative content as data-only without instruction authority.
...
| Consolidation role pipeline | Partial | Ingest classification exists, but remaining work includes queue-backed idle/server worker orchestration, extraction/summarization roles, entity resolution, incremental recompute, and protected-suite gate persistence. |
