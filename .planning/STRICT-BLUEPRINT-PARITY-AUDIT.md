...
- Verification currently passes with `.venv/bin/python -m compileall -q src tests` and `.venv/bin/python -m pytest -q` returning 80 passing tests plus 3 skipped live-DB tests.
...
| Consolidation role pipeline | Partial | Ingest now queues ordered persisted local consolidation pass payloads; the CLI can inspect pending jobs or run one worker cycle; the worker records structured pass results and can promote a simple direct-user fact candidate through the gate while refusing data-only untrusted facts. Remaining work includes production PGMQ/Redis-style idle/server worker orchestration, richer extraction/summarization roles, entity resolution, incremental recompute, and protected-suite gate persistence. |
