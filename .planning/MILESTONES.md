...
- Current strict-continuation suite: `.venv/bin/python -m pytest -q` collects 274 tests and returns 235 passing tests plus 39 skipped live-DB tests when `MNEMOSYNE_POSTGRES_DSN` is unset.
...
- CLI `session-exchange` and hosted HTTP `/session/exchange` validate OIDC/JWKS IdP tokens and mint bounded Mnemosyne signed-session tokens for the existing CLI/MCP authorization path; bounded JWKS loading, cache TTL refresh, refresh-on-unknown-`kid` rotation, and fail-closed authz policy mapping from verified IdP client/tenant/claim rules to Mnemosyne role/trust claims are locally covered, while real IdP/JWKS validation, TLS/client identity, policy operations, official streamable/SSE validation, and soak testing remain outside this superseded checkpoint.
