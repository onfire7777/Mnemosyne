...
- CLI `deployment-soak` runs allowlisted deployment preflight manifests, including IdP authz rollout and TLS rotation-plan checks, through shell-free subprocesses, parses child JSON reports, omits command arguments, and fails closed on required failures/timeouts/non-JSON/disallowed commands.
