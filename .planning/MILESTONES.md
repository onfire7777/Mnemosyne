...
- Current strict-continuation suite: `.venv/bin/python -m pytest -q` collects 266 tests and returns 227 passing tests plus 39 skipped live-DB tests when `MNEMOSYNE_POSTGRES_DSN` is unset.
...
- CLI `session-exchange` and hosted HTTP `/session/exchange` validate OIDC/JWKS IdP tokens and mint bounded Mnemosyne signed-session tokens for the existing CLI/MCP authorization path; bounded JWKS loading, cache TTL refresh, and refresh-on-unknown-`kid` rotation are locally covered, while production policy mapping, real IdP/JWKS validation, TLS/client identity, official streamable/SSE validation, and soak testing remain outside this superseded checkpoint.
