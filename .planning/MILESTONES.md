...
- Current strict-continuation suite: `.venv/bin/python -m pytest -q` collects 258 tests and returns 219 passing tests plus 39 skipped live-DB tests when `MNEMOSYNE_POSTGRES_DSN` is unset.
...
- CLI `session-exchange` and hosted HTTP `/session/exchange` validate OIDC/JWKS IdP tokens and mint bounded Mnemosyne signed-session tokens for the existing CLI/MCP authorization path; production policy mapping, JWKS rotation/cache operations, TLS/client identity, official streamable/SSE validation, and soak testing remain outside this superseded checkpoint.
