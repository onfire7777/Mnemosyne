...
**Status:** production-attested after complete planning, architecture, additive
infra scaffold, live bring-up, and evidence capture. **Defers to:** the blueprint, live repo evidence, the
...
*trustworthy* completion. The self-hosted no-GPU profile now closes **B1-B10**
on real retained evidence. **B9/FR-21 is not closed by declaration**; it is
closed by ADR-002's amended CPU-trained parametric adapter evidence path. No
gate weakened; §31 rails + §16 SLOs preserved.
...
- **B9/strict-100%:** closed by retained CPU-parametric evidence through ADR-002's amended path; cloud/GPU remains optional scale, never a substitute for evidence.
...
1. **Maintain the ledger**: Phase 8 is recorded in `.planning/ROADMAP.md`, `.planning/MILESTONES.md`, and `.planning/STATE.md`; keep those entries subordinate to `.planning/ACTIVE-GOAL-OPERATING-CONTRACT.md` and preserve the retained production evidence that marks Tier-B Done.
...
5. **Capture B1-B10** via `infra/PRODUCTION-EVIDENCE.md` (render -> preflight -> capture -> release-audit -> verify); this is complete for `capture-bc10`.
6. **B9/FR-21:** preserve the retained CPU-parametric evidence and use the `cloud` profile only for larger future trainer scale. Do not fake it.
7. **Tier C:** preserve the release attestation and rerun SLO/LongMemEval checks only when planning a new release cut.
...
The infra is now **production-attested for Tier-B** by `capture-bc10`. For any
future recapture, do not mark anything Done until its `*-ops-check` passes on
real infra and the full wrapper/audit/verifier path passes.
