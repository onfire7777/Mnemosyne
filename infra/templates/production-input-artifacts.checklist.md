...
- For `provenance-trust-suite.json`, nested `asset_path` and `c2pa_asset_path`
  ...
  external directory.
- Run `infra/scripts/render-production-soak-manifest.sh --env-file /secure/path/to/mnemosyne-tier-b-custody/production-render.env --runtime-env-file "$RUNTIME_ENV_FILE" --check-environment`
...
- For secret-bearing provider/runtime variables, prefer passing a separate
  ...
  `infra/scripts/render-production-soak-manifest.sh --runtime-env-file "$RUNTIME_ENV_FILE"`
  for readiness and
