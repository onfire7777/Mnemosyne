...
- Start with an external no-secret custody packet instead of hand-assembling the
  ...
  writes `reports/provider-env-action-plan.{json,md}` as the provider-stack
  handoff that maps every `provider-manifest.production.json` env reference to
  its manifest JSON path, provider check, primary row ownership, and shared
  provider-check blast radius without retaining values or runtime env-file
  ...
  runbooks, and consuming checks. The paired
  `reports/input-artifact-contracts.{json,md}` report records each artifact's
  kind, consuming validators, advisory validator section/check hints,
  release-audit output-key contract, and minimum operator contract before files
  are supplied. These are not evidence; do not create placeholder JSON, PEM, or
  bundle files to make readiness pass. The adjacent
...
- The manifest must include the exact production release profile: every command in the current 29-command set from `src/mnemosyne/cli.py`, with no duplicate or unknown commands.
...
with `--runtime-env-file "$RUNTIME_ENV_FILE"`.
