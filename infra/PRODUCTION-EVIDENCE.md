...
- Start with an external no-secret custody packet instead of hand-assembling the
  ...
  kind, consuming validators, release-audit output-key contract, and minimum
  operator contract before files are supplied. These are not evidence; do not
  create placeholder JSON, PEM, or bundle files to make readiness pass. The adjacent
