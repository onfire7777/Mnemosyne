...
## Follow-up fix: categorical encrypted-key rejection

### Root cause and scope

Independent review found that the generic validator still invoked
`openssl pkey -passin pass:`. OpenSSL therefore accepted a genuine encrypted
PKCS#8 key whose password was empty, allowing it to reach the key-match success
path. The bounded fix rejects the two encrypted-PEM indicators before any
private-key parse: `BEGIN ENCRYPTED PRIVATE KEY` and legacy
`Proc-Type: 4,ENCRYPTED`. It preserves non-interactive parsing for malformed or
otherwise unreadable unencrypted keys and emits only the sanitized failure
`private key must not be encrypted`.

No live secret, runtime, model, graph/index, benchmark, evaluation, exact-scale,
held-out, or protected operation was performed.

### Targeted hardware admission

Exact samples, taken 15 seconds apart:

```text
sample=1 timestamp=2026-07-13T06:23:08Z
memory_free_percent=46%
load_averages=4.14 4.30 4.08
resident_model_rows=0
sample=2 timestamp=2026-07-13T06:23:23Z
memory_free_percent=45%
load_averages=4.88 4.46 4.15
resident_model_rows=0
sample=3 timestamp=2026-07-13T06:23:38Z
memory_free_percent=44%
load_averages=4.68 4.44 4.14
resident_model_rows=0
```

All three samples passed the targeted-only floor of at least 35% free memory,
one-minute load at most 10, and zero resident models. They did not satisfy or
claim the 55% heavy-work gate.

### RED evidence

The empty-password fixture was generated by the selected OpenSSL executable
with `pkcs8 -topk8 -v2 aes-256-cbc -passout pass:`. Before the regression was
added, an isolated probe returned this exact output and exit status:

```text
marker=-----BEGIN ENCRYPTED PRIVATE KEY----- parse_exit=0
```

Exit status: `0`.

The regression was then added before production code changed and run exactly:

```bash
python -m pytest -q \
  tests/test_production_vault_tls_validator.py::test_production_tls_validator_rejects_empty_password_encrypted_private_key
```

Exit status: `1`. Exact failure-bearing stdout excerpt:

```text
F                                                                        [100%]
>       assert proc.returncode == 65
E       AssertionError: assert 0 == 65
E        +  where 0 = CompletedProcess(args=['/Users/admin/Mnemosyne/infra/validate/validate-production-vault-tls.sh', '/private/var/folders/c9/0hq50vks4fj_mr_5zfqkgj380000gn/T/.ctx-mode-psMT56/pytest-of-admin/pytest-0/test_production_tls_validator_0/vault-root.crt', '/private/var/folders/c9/0hq50vks4fj_mr_5zfqkgj380000gn/T/.ctx-mode-psMT56/pytest-of-admin/pytest-0/test_production_tls_validator_0/vault.crt', '/private/var/folders/c9/0hq50vks4fj_mr_5zfqkgj380000gn/T/.ctx-mode-psMT56/pytest-of-admin/pytest-0/test_production_tls_validator_0/vault.key'], returncode=0, stdout='production Vault TLS chain verified\n', stderr='').returncode
tests/test_production_vault_tls_validator.py:344: AssertionError
FAILED tests/test_production_vault_tls_validator.py::test_production_tls_validator_rejects_empty_password_encrypted_private_key
```

The command captured stderr into stdout with `2>&1`; the shell wrapper itself
emitted no separate stderr. The failure was the intended missing-behavior
failure: the old validator returned success rather than the required exit 65.

The pre-existing password-encrypted PKCS#8 expectation and a real legacy
OpenSSL `EC PRIVATE KEY` fixture containing `Proc-Type: 4,ENCRYPTED` were also
set to the categorical sanitized error before production changed. Their exact
single-test runs each exited `1`; the old code returned respectively the old
`private key could not be read` error in both cases.

### GREEN evidence

After adding only the encrypted-PEM marker guard, the three encrypted-key cases
were run exactly:
...
python -m pytest -q \
  tests/test_production_vault_tls_validator.py::test_production_tls_validator_rejects_encrypted_private_key_without_prompt \
  tests/test_production_vault_tls_validator.py::test_production_tls_validator_rejects_empty_password_encrypted_private_key \
  tests/test_production_vault_tls_validator.py::test_production_tls_validator_rejects_legacy_encrypted_private_key
```
...
Exact stderr: empty. The focused validator/bootstrap result is 22/22 passed.

### Follow-up security sweep

The exact three changed tracked files were scanned for risky filenames, private
key blocks, AWS/GitHub/OpenAI credential forms, JWT-shaped values, and files
over 50 MiB. Exact derived result:

```text
changed_files=.superpowers/sdd/task-phase12-tls-validator-hardening-report.md,infra/validate/validate-production-tls.sh,tests/test_production_vault_tls_validator.py
credential_findings=none
max_size_bytes=20685
```
