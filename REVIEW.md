...
- Parametric follow-up: command-backed LoRA/test-time-training adapter providers now run through CLI/MCP configuration, require operator-grade authorization, fail closed on provider contract errors, enforce local mutation-rate/reward/sink/gate rails, and persist proposal/evaluation/rollback provider payloads.
