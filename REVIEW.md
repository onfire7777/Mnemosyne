...
- KMS follow-up: encrypted object storage now supports a command-backed envelope-key manager for KMS/HSM/Vault-style deployments, with CLI and MCP coverage for key creation, lookup, existence, and shredding through the command boundary.
...
- Provenance follow-up: C2PA trust policy now supports trusted certificate-root SHA-256 fingerprints globally and per scoped rule, and rejects valid manifests whose certificate chain root does not match the configured trust roots.
- Parametric follow-up: command-backed LoRA/test-time-training adapter providers now run through CLI/MCP configuration, require operator-grade authorization, fail closed on provider contract errors, and persist proposal/evaluation/rollback provider payloads without bypassing current local source/rail/gate checks.
