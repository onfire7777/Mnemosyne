...
def test_unimplemented_surfaces_name_their_task(tmp_path: Path):
    engine = SqliteEngine(tmp_path / "root")
    # Task 3 ledger + Task 4 scan surfaces (vector_search/lexical_search/
    # graph_ppr) are implemented; only Task 5+ write/pipeline surfaces remain
    # stubbed. (Task 4: scan surfaces landed.)
    with pytest.raises(NotImplementedError, match="Task 5"):
        engine.upsert_assertion(None)  # type: ignore[arg-type]
