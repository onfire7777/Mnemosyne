...
def test_production_evidence_docs_point_to_generated_next_commands_script() -> None:
    for path in [
        ...
        assert "reports/render-env-action-plan" in text, path
        ...
        assert "section/check hints" in text, path
...
def test_current_state_docs_do_not_reopen_closed_local_feature_gaps() -> None:
    ...
    assert "GitHub Actions CI run `28475668973` passing" not in handoff_flat
