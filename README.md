...
The self-test exercises `initialize`, `tools/list`, strict MCP input schemas, auth-token rejection, signed-session enforcement, and a read-only tool call. It redacts configured secrets and does not replace real IdP, production certificate rotation/lifecycle automation, or production endpoint soak runs.
...
- `.venv/bin/python -m pytest -q` collects 295 tests and returns 256 passing tests plus 39 skipped live-DB tests when `MNEMOSYNE_POSTGRES_DSN` is unset.
