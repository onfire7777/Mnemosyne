...
Use `provider-check` with the same flags for deployment smoke checks. It returns structured JSON and exits nonzero if any embedding, reranker, media-extractor, or command parametric-provider contract fails.
...
- `uv run pytest -q` collects 167 tests and returns 146 passing tests plus 21 skipped live-DB tests with the optional MCP SDK extra installed.
