...
**The trap to avoid (already observed):** Codex has been spending recent cycles adding more `test(...)` coverage and more release-audit *gates*. With the mandatory Tier A wirings now closed, placeholder/hollow evidence rejected, production manifest rendering made deterministic/fail-closed, and offline custody review wired, real progress should pivot to **Tier B real-infrastructure evidence** — and optional A11/A12 only if a concrete audit finding demands them — not more gates or tests.
...
| A11 | **Hosted-MCP transport** (FR-9) | Local JSON-RPC/TLS/self-test done; 2026-06-25 local readiness passed hosted JSON-RPC HTTP, SDK StreamableHTTP, and SSE-compatible CLI soaks. | Capture hosted operator endpoint evidence in Tier B; write more transport code only on a concrete production-evidence or strict-audit finding. | Local soaks green; production endpoint evidence still required. | **M** |
...
| B3 | CLI/MCP runtime coverage | Official **StreamableHTTP/SSE** transport + stateless soak on a real deployed endpoint | Full 28-command production manifest via `infra/PRODUCTION-EVIDENCE.md`; row signal: hosted MCP soak evidence |
