...
**Authored:** 2026-06-24 · **Last updated:** 2026-06-27 · **Current baseline:** `main` after G1 reliability-core wiring, G4 shadow workspace/advisory/retrieval-controller gates, production manifest rendering, operator handoff hardening, offline custody verification, and G0 resource-cost instrumentation
...
| B8 | Observability dashboards | Real hosted dashboard with tripwires, freshness, access controls, alert routing, and source snapshot fingerprinting; package mode is allowed only as pre-existing external custody input evidence, not generated by `ops-report --dashboard-package-dir` during capture | Full 28-command production manifest via `infra/PRODUCTION-EVIDENCE.md`; row signal: `ops-dashboard-check` |
