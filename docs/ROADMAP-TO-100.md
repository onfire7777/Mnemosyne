...
This is the **bulk of the remaining percentage** and the universal blocker on all 10 audit rows. The code paths, adapter boundaries, evidence gates, capture wrapper, custody manifest, redaction scan, and offline bundle verifier already exist; `infra/` already scripts Keycloak/Vault/c2patool. Local real-service evidence is now captured for Keycloak, Vault transit, retrieval-provider metadata, and C2PA trust verification; what's still missing is **operator-captured production evidence bundles**.
