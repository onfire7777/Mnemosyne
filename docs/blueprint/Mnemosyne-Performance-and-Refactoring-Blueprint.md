...
7. **Benchmark posture — Public-Benchmark Publication Protocol (PBPP).** Public benchmark numbers are permitted only under `docs/EXECUTION-PLAN-B-Benchmark-and-Leaderboard.md` §2: the pinned `eval/public/` harness produces the number; the complete artifact bundle ships with it; retrieval-recall and LLM-judged QA remain separate columns with the judge model and prompt disclosed; the private golden suite is never conflated with public results; and an independent third party reproduces the number from the bundle before any headline claim. Private-suite numbers remain internal QA and are never public headline claims. Track **LongMemEval + BEAM/ConvoMem** under that protocol, treat **LoCoMo as contested** (Zep's critique: answer leakage, short horizons), and never headline it.
