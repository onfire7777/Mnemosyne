...
2. **Make the native Rust kernels the standard tier and finish release-wheel delivery** (PARITY-SAFE). The scalar byte-parity kernels are already measured at 41× (lexical), 12× (PPR), ≥10× (dense) over the pure-Python fallback while bit-identical, and the current macOS arm64 / Linux x86_64 wheel builders are now merge-gating with built-wheel install/import smoke proof. The remaining release gap is broader wheel matrix coverage and default distribution so most installs stop silently running the slow path. Realizing acceleration that already exists is the cheapest engine-side win.
...
4. **Backfill null embeddings, close `embedding_partition='none'` coverage, and cap the Postgres dense fallback** (PARITY-SAFE for scoring). Rows in the `none` partition are deliberately non-embeddable and should not receive dead HNSW coverage; the source-owned fix is btree coverage for lifecycle/audit scans plus an embeddable-null fallback index that excludes `none` before the per-query Python embed loop. `ops-report` now exposes both the hygiene gate and a redacted backfill plan for operator evidence correlation. Remaining work is executing and retaining production backfill/alerting evidence.
5. **Finish the caching lattice around the caches already landed** (PARITY-SAFE — caching returns identical bytes). The hashing-embedding path is LRU-cached (`text.py:96`), `HttpEmbeddingProvider` has a process-local content-hash LRU keyed by cache-scope/provider/model/revision/dims/API-key hash/text SHA-256 and an optional SQLite durable cache path with TTL, hashed identity storage, redaction-gated durable writes for secret-like input, and `provider-check` cache status (`retrieval.py`), SqliteEngine carries the subject-scoped A1 `embedding_cache` table, the retrieval pipeline has a default-off full-result LRU, and Postgres caches positive calibration lookups. Remaining work is production cache-hit/latency evidence plus negative/abstention caching for provider responses that actually return abstentions or negative results.
6. **Adopt a production-grade embedding server (HuggingFace Text-Embeddings-Inference) with token-based dynamic batching, colocated with the engine** (APPROX-LAYER only if the model changes; PARITY-SAFE if the model/precision is held constant) — and run the `rust/mneme-providers` sidecar bake-off that the native-acceleration program requires before any default change. The Python service, HTTP adapters, and Rust sidecar now share a fixture-backed conformance lane; the bake-off/default decision remains open.
7. **Fast approximate shortlist + exact rescoring on Postgres**: `halfvec` HNSW index and pgvector 0.8 iterative scans for candidate generation, then exact `f64` cosine rescoring on the shortlist (APPROX-LAYER for the index, PARITY-SAFE for the returned scores). Optional `pgvectorscale` StreamingDiskANN for large tenants. Postgres server tuning is now versioned as `infra/postgres/postgresql-perf.conf`, but applying it and claiming a win still requires retained before/after operator evidence.
...
| B2 | Native kernels exist and current two-runner wheel builds are gated with built-wheel install/import smoke proof, but full release-wheel delivery/default install is incomplete, so most installs still run pure Python | CONCERNS §Deps; `.github/workflows/ci.yml` | §7.1 native-default + wheels-as-gate | PARITY-SAFE |
...
| B17 | **`embedding_partition='none'` rows are deliberately non-embeddable and need explicit non-vector coverage** — HNSW remains correct only for `public`/`private`; `none` rows need btree lifecycle/audit coverage and must be excluded from the embeddable null fallback | `sql/schema.sql`; `postgres_engine.py` | §7.4 source coverage fix | PARITY-SAFE |
...
| B7 | Query-layer HNSW knobs are now set (`hnsw.ef_search` and `hnsw.iterative_scan` before pgvector queries), and a conservative Postgres tuning profile is versioned, but **no live before/after tuning evidence has been captured yet** | `postgres_engine.py:351`; `infra/postgres/postgresql-perf.conf`; `docker-compose.prod.yml:109-110` | §7.4 server knobs + prepared statements | APPROX-LAYER / PARITY-SAFE (knobs) |
| B8 | ~~No embedding cache~~ **(corrected)** Hashing-embedding LRU exists (`text.py:96`, 8192 entries), `HttpEmbeddingProvider` has process-local and optional durable privacy-scoped/TTL cache paths (`retrieval.py`), and SqliteEngine has the subject-scoped A1 `embedding_cache` table (`sqlite_schema.py:254-266`); **production cache-hit/latency evidence plus negative/abstention cache evidence remain open** | `retrieval.py`; `CONFIG-DRIFT-CHECKS.md`; `tests/test_provider_batching.py` | §7.8 durable provider cache + cache-safety suite | PARITY-SAFE |
...
| B13 | Stateless MCP mode now reuses same-scope warm tool bundles and invalidates local durable stores on file changes; production latency evidence is still required | `mcp_server.py` (`_stateless_tools_for`), `tests/test_runtime_surfaces.py` | §7.7 warm reuse / keepalive | PARITY-SAFE |
...
2. **Run the `mneme-providers` bake-off the native-accel program already requires** (PARITY-SAFE decision input). The sidecar is built but undefaulted for lack of evidence. Bench it against `services/embedding` (fp32 torch) and TEI on the same model/hardware; the wire contract now accepts list-input `/embed`, and `eval/provider_bakeoff/run.py` packages retained SLO/provider-check/noise evidence without authorizing a smoke-fixture promotion, but the model backend still needs true throughput batching if it is to compete seriously. Whichever provider wins becomes the *recommended* deployment; the losers remain contract-conformant options (§7.11.1 now gates the shared fixture in CI).
...
**Current state.** Production schema is `VECTOR(1024)` with HNSW (`vector_cosine_ops`) on `evidence.embedding` and `assertions.embedding`, correctly partial over vector-bearing `embedding_partition IN ('public','private')`. The `none` partition is non-embeddable by policy: write paths and the schema reconciler clear its vector, and dense retrieval excludes it. Source coverage now uses btree partial indexes for `none` lifecycle/audit scans plus `evidence_null_embedding_fallback_idx` for embeddable legacy rows with `embedding IS NULL`. Tenant RLS, FTS (stored `lexeme` TSVECTOR + GIN — landed in P3), recursive PPR with the persisted `graph_ppr_cache`. Connection pooling landed in P3 (`postgres_engine.py:121,183`), and hot FTS/pgvector retrieval SELECTs now force Psycopg server-side preparation behind `MNEMOSYNE_PG_PREPARE_HOT_QUERIES=0`. The residual `embedding IS NULL` fallback is capped and excludes `none`; `ops-report` now exposes a read-only Postgres vector hygiene snapshot plus `--require-clean-vector-hygiene` for operator gating across evidence and assertions, and can include a redacted backfill plan with candidate row ids and hashes for pre-backfill evidence correlation. `vector-backfill-apply` is now the guarded source-owned execution path: it requires `--confirm-apply`, applies bounded batches, redacts raw row text, and stores both `public` and `private` assertion vectors while leaving `none` NULL. Production backfill evidence remains open until retained operator output and a clean post-backfill hygiene probe are captured. Server tuning is versioned in `infra/postgres/postgresql-perf.conf`; it is not applied silently, and before/after operator evidence is still required.

**Work items:**

1. **Backfill null embeddings during ingestion/consolidation and cap the fallback** (B3; APPROX-LAYER for the index, PARITY-SAFE for scoring). Make embedding at write time the contract so the read path never runs the unbounded per-query Python embed loop. The fallback is now capped/paginated and `ops-report --require-clean-vector-hygiene` treats nonzero evidence/assertion embeddable NULL-vector backlog as a report-failing production alert. `postgres_vector_hygiene.backfill_plan` adds read-only, redacted row-id/hash samples for retained pre-backfill evidence, and `vector-backfill-apply` provides the explicit bounded apply path; the remaining work is to run/retain production backfill output and a clean post-backfill hygiene probe.
2. **Landed source slice: close `embedding_partition='none'` coverage without dead HNSW** (B17; PARITY-SAFE). `none` means non-embeddable, so HNSW remains limited to `public`/`private`. Ordinary btree partial indexes cover `none` lifecycle/audit scans, and the embeddable null fallback has its own partial btree index plus a SQL predicate excluding `none` before Python embedding. Future halfvec/pgvectorscale work must keep `none` as a no-vector partition unless a new ADR changes the access-policy model.
3. **Landed: prepared statements + plan reuse for hot retrieval SELECTs on pooled connections** (B6-residual; PARITY-SAFE). FTS and pgvector shortlist queries pass Psycopg `prepare=True`, while tenant and HNSW `set_config(..., true)` statements remain explicit and transaction-local. `MNEMOSYNE_PG_PREPARE_HOT_QUERIES=0` is the rollback/PgBouncer compatibility switch. Future work can extend preparation to append/write-heavy statements after live production measurement.
...
7. **Postgres server tuning file** (PARITY-SAFE). A version-controlled `infra/postgres/postgresql-perf.conf` now records conservative values for `shared_buffers`, `maintenance_work_mem`, `work_mem`, `effective_cache_size`, `max_parallel_workers_per_gather`, and `jit=off`, with `hnsw.ef_search` deliberately left query-local. Pair every knob with a before/after bench row before applying it to the live stack; the file is a candidate, not evidence by itself.
...
1. **Unify the provider contract (three implementations, two languages).** The `/embed` + `/rerank` + `/health` HTTP contract is implemented three times: `services/embedding/app.py` (Python real + deterministic fallback), `rust/mneme-providers` (axum/fastembed), and the in-process deterministic fallbacks in `retrieval.py` — with TEI arriving as a fourth conformer (§7.2.1). The first conformance slice now shares `tests/fixtures/provider_contract.json` across the Python service, Python HTTP adapters, and Rust sidecar tests, and CI has a dedicated `provider-conformance` lane. Remaining unification work is a versioned schema/spec with broader property checks, health/auth coverage, and a bake-off-decided primary per deployment profile. Contract drift across provider codebases is now a red CI lane instead of an untracked quality risk.
...
4. **Decouple the index layer from the access-policy vocabulary.** HNSW indexes are physically partitioned by `embedding_partition` (public/private) — correct for security today, but it couples storage DDL to the policy model: a new vector-bearing sensitivity partition would force an index migration. `none` is now explicitly modeled as the no-vector partition with btree coverage (ADR-003). Evaluate replacing partition-partial HNSW indexes with (a) a single covering HNSW index + iterative-scan filtering (§7.4.5), or (b) a generated column that *guarantees* vector-bearing coverage — decide on measured recall/latency under RLS, and record as an ADR. The access-policy semantics themselves do not change.
...
7. **Benchmark posture — Public-Benchmark Publication Protocol (PBPP).** Public benchmark numbers are permitted only under `docs/EXECUTION-PLAN-B-Benchmark-and-Leaderboard.md` §2: the pinned `eval/public/` harness produces the number; the complete artifact bundle ships with it; retrieval-recall and LLM-judged QA remain separate columns with the judge model and prompt disclosed; the private golden suite is never conflated with public results; and the bundle makes the result reproducible by construction before any headline claim. Independent third-party reproduction remains welcome evidence that can strengthen a claim, not a prerequisite for publication. Private-suite numbers remain internal QA and are never public headline claims. Track **LongMemEval + BEAM/ConvoMem** under that protocol, treat **LoCoMo as contested** (Zep's critique: answer leakage, short horizons), and never headline it.
...
- Provider bake-off: `services/embedding` vs TEI vs `mneme-providers` vs in-process ONNX; the shared contract conformance lane now gates Python service/adapters and the Rust sidecar in CI, and `eval/provider_bakeoff/run.py` now packages retained eval/provider-check/noise evidence into a promotion-safe report, while real bake-off/default-selection evidence remains pending (§7.2.1–7.2.4, §7.11.1).
- Null-embedding backfill source path + fallback cap; `'none'`-partition index coverage; prepared statements; Postgres server tuning; `halfvec` index + exact rescoring (§7.4). Query-layer `ef_search` and iterative scans are already wired and remain part of the perf-lane contract; production backfill execution evidence remains operator-retained.
...
| `services/embedding/`, `rust/mneme-providers/`, `tests/fixtures/provider_contract.json` | Shared provider contract conformance lane; remaining provider bake-off, sidecar throughput batching, TEI deploy path, and versioned schema/spec | §7.2, §7.11.1 |
...
2. Provider primary: TEI, the repaired `mneme-providers`, or in-process ONNX for single-node self-hosted — decided by a retained `eval/provider_bakeoff/run.py` Wave-B bake-off report, recorded as an ADR.
