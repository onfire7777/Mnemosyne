...
2. **Make the native Rust kernels the standard tier and gate their wheels in CI** (PARITY-SAFE). The scalar byte-parity kernels are already measured at 41× (lexical), 12× (PPR), ≥10× (dense) over the pure-Python fallback while bit-identical — but wheels are non-gating today, so most installs silently run the slow path. Realizing acceleration that already exists is the cheapest engine-side win.
...
5. **Complete the caching lattice: content-hash real-model embedding cache, result cache, and calibration cache** (PARITY-SAFE — caching returns identical bytes). The hashing-embedding path is already LRU-cached (`text.py:96`) and SqliteEngine carries the subject-scoped A1 `embedding_cache` table; the missing piece is a `(model_id, model_rev, text_sha256)`-keyed cache in front of `HttpEmbeddingProvider` for the Local/Postgres paths, where embedding inference dominates warm `fast_path_total`.
...
| B2 | Native kernels exist but wheels are **non-gating**, so most installs run pure Python | CONCERNS §Deps; `.github/workflows/ci.yml` | §7.1 native-default + wheels-as-gate | PARITY-SAFE |
...
| B5 | GIL serializes native compute; parallel channels are opt-in and off by default | `pipeline.py:55-65` (`MNEMOSYNE_PARALLEL_CHANNELS`) | §7.1 GIL release; §7.5 parallel-by-default on capable tiers | PARITY-SAFE |
...
| B7 | HNSW `ef_search` not tuned per query class; no iterative index scans under filters; **no Postgres server tuning beyond `shared_buffers`/`maintenance_work_mem`** | external pgvector 0.8 refs; `docker-compose.prod.yml:109-110`; `infra/postgres/` has no conf file | §7.4 ef_search + iterative scans + server knobs | APPROX-LAYER / PARITY-SAFE (knobs) |
| B8 | ~~No embedding cache~~ **(corrected)** Hashing-embedding LRU exists (`text.py:96`, 8192 entries) and SqliteEngine has the subject-scoped A1 `embedding_cache` table (`sqlite_schema.py:254-266`); **the real-model HTTP path (Local/Postgres) has no content-hash cache**, and query embeddings recompute per repeated query | `retrieval.py` (no cache around `HttpEmbeddingProvider`); `engine.py:2868,2903` | §7.8 content-hash + query-embed cache | PARITY-SAFE |
...
**Current state.** `run_retrieval_pipeline()` runs dense/lexical/graph channels; parallelism is opt-in via `MNEMOSYNE_PARALLEL_CHANNELS` (`ThreadPoolExecutor(max_workers=3)`, `pipeline.py:55-65,146`), default off. RRF/MMR are in `algorithms.py` (native-mirrored; RRF shallow-reconstruction already landed in P3, killing the `copy.deepcopy` tax). PostgresEngine MMR re-hashes text into hashing-space instead of reusing stored pgvector vectors; the stored-vector MMR fix exists but is gated opt-in (`postgres_engine.py:4128`, commit `b8347b7`).

**Work items (all PARITY-SAFE unless noted):**

1. **Enable parallel channels by default on `standard`/`accelerated` tiers** (B5). With the GIL released around native compute (§7.1) and I/O-bound SQL/HTTP channels, overlapping the three channels is a strict latency win. `capability.py` already *recommends* `MNEMOSYNE_PARALLEL_CHANNELS=1` for accelerated tiers — make it the resolved default there, keep it off on `floor`.
...
**Current state (corrected in v1.1).** Three cache surfaces already exist: the hashing-embedding LRU (`text.py:96`, 8192 entries, backend-agnostic by design), the SqliteEngine subject-scoped A1 `embedding_cache` table (`sqlite_schema.py:254-266`, privacy-aware admit/purge logic in `sqlite_engine.py`), and `GraphSignalCache` (per tenant/branch graph hits) plus the persisted `graph_ppr_cache` table. **Missing:** any cache in front of the real-model HTTP embedding path used by Local/Postgres, any result cache, and any calibration-lookup cache.

**Work items (all PARITY-SAFE — a hit returns identical bytes to a miss):**

1. **Content-hash real-model embedding cache** keyed on `(model_id, model_revision, sha256(normalized_text))`, fronting `HttpEmbeddingProvider` for both query and document embeddings. Embedding is deterministic for a fixed model+revision, so this is exact. It removes the dominant warm cost (B1/B8) for repeated and templated queries, and turns deliberate re-projection passes from O(embed) into O(hash lookup). Respect the same subject-scoped admit/purge policy the A1 SQLite cache already implements (privacy tightening purges the row); seed honeytokens into the cache namespace (§4.5).
2. **Result cache** keyed on the **full** request context: `(tenant, branch, query, k, route, sensitivity_ceiling, as_of, policy_version, calibration_version)`. Must cache the **post-sanitization, post-redaction** `RetrievalResult` so §31 R6 and access-policy boundaries are never bypassed. Invalidate on any write to the tenant/branch (evidence append, supersession, forget) and on policy/calibration version bumps.
3. **Calibration lookup cache** — conformal calibration parameters per tenant/memory-type are read on every scored answer; cache the resolved calibrator with invalidation on `conformal_calibration` updates.
...
- Content-hash embedding cache + calibration cache (§7.8.1, 7.8.3), with the byte-identity cache-safety test and honeytoken seeding.
...
- Null-embedding backfill + fallback cap; `'none'`-partition index coverage; prepared statements; per-route `ef_search`; iterative scans; `halfvec` index + exact rescoring (§7.4).
- Result cache with full-context key + write invalidation (§7.8.2).
