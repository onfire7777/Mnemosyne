...
2. **Run the `mneme-providers` bake-off the native-accel program already requires** (PARITY-SAFE decision input). The sidecar is built but undefaulted for lack of evidence. Bench it against `services/embedding` (fp32 torch) and TEI on the same model/hardware; the wire contract now accepts list-input `/embed`, but the model backend still needs true throughput batching if it is to compete seriously. Whichever provider wins becomes the *recommended* deployment; the losers remain contract-conformant options (§7.11.1 now gates the shared fixture in CI).
...
- Provider bake-off: `services/embedding` vs TEI vs `mneme-providers` vs in-process ONNX; the shared contract conformance lane now gates Python service/adapters and the Rust sidecar in CI, while bake-off/default-selection evidence remains pending (§7.2.1–7.2.4, §7.11.1).
