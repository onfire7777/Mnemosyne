...
7. **Benchmark posture.** Track **LongMemEval + BEAM/ConvoMem** as the internal sanity gates; treat **LoCoMo as contested** (Zep's critique: answer leakage, short horizons) and never headline it. Per the honesty charter, public claims remain measured-latency/SLO numbers only — these suites gate regressions, they do not market. *(Posture evolution: `docs/EXECUTION-PLAN-B-Benchmark-and-Leaderboard.md` proposes upgrading this rule into the Public-Benchmark Publication Protocol — public numbers become permitted, but only under its stricter pinned-harness / artifact-bundle / third-party-reproduction discipline. Its Task M4 amends this section when ratified.)*
