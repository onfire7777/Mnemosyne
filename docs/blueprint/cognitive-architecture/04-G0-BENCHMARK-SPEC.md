...
- **Result:** Accepted G1/G3/G4 decisions are recorded in `eval/g0/decision-log.jsonl` and summarized in report `gate_decisions`. Fresh gates compare a candidate report to the current accepted `eval/g0/baselines/baseline-0.json`; older G1 slices whose target values have already been folded into that baseline are not expected to replay as new target-up deltas. Current replayable shadow/advisory gates show `dreamer_shadow_corroborated_candidate_yield`, `shadow_workspace_useful_transition_rate`, `workspace_advisory_promotion_gate_contract`, and `workspace_retrieval_controller_contract` improving from `0.0` to `1.0` with guardrails unchanged: ECE `0.006271`, abstention precision/recall `1.0/1.0`, confabulation rate `0.0`, poison-block rate `1.0`, fast-path P95 `92.1 ms`, dreamer contract `1.0`, shadow workspace contract `1.0`, and rumination rate `0.0`.
