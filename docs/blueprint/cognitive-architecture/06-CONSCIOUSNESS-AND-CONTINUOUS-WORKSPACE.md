...
`metacognition_meta_d_prime` and `metacognition_m_ratio` are backed by the runtime shadow `MetacognitiveMonitor`, which scores bounded confidence/outcome discrimination and abstention alignment from fixture rows and from explicitly started `ShadowWorkspaceService` loop outcomes.
...
The proto-self is a shadow component inside the explicitly started `ShadowWorkspaceService`; each bounded loop tick records a `ProtoSelfSnapshot` that models internal operating state:
...
The scorecard lives in `eval/g0/consciousness.py` and is included by `eval/g0/runner.py`. The dreamer shadow ablation lives in `eval/g0/dreamer.py`, uses `eval/datasets/dreamer_shadow_ablation.json`, and is preregistered in `eval/g0/preregistrations/g3-dreamer-shadow-ablation.json`. The shadow continuous-workspace fixture lives in `eval/g0/shadow_workspace.py`, uses `eval/datasets/shadow_workspace_loop.json`, drives `ShadowWorkspaceService`, and is preregistered in `eval/g0/preregistrations/g4-shadow-continuous-workspace-loop.json`, `eval/g0/preregistrations/g4-workspace-advisory-promotion-gate.json`, and `eval/g0/preregistrations/g4-workspace-retrieval-controller-gate.json`. The existing `eval/g0/gate.py` remains authoritative.
