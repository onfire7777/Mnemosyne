...
`shadow_workspace_useful_transition_rate`, `shadow_workspace_contract`, `workspace_consolidation_advisory_contract`, `workspace_advisory_promotion_gate_contract`, and `shadow_workspace_rumination_rate` are backed by the G4 shadow continuous-workspace fixture in `eval/g0/shadow_workspace.py`; they measure bounded useful state progression, cycle consistency, shadow-only safety, default-off advisory behavior, explicit opt-in advisory promotion through prediction/replay inputs, cross-tenant/source-CID rejection, and anti-rumination exits.
