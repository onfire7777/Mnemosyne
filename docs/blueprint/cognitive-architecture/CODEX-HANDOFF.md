...
**Current handoff snapshot:** 2026-06-29 UTC. This snapshot includes §24 support-strategy runtime durability plus public CLI/MCP mutation surfaces, Row 08 hosted-dashboard release-audit hardening, runtime protected `T-SEC` registry reconciliation, protected gate-case ratchet enforcement, replay-fidelity report hygiene, tenant-scoped Postgres gate-case mirror IDs for public non-UUID cases, OQ3 dirty-source projection recompute cleanup, terminal shadow-workspace hard stops, hosted-provider evidence hardening, structured evidence redaction, refreshed G0 report custody, and the latest verified pushed baseline (`998ac09`, CI run `28344470006`) while preserving the production release-custody and stateful workspace-service hardening already on `main`; strict v1.0 parity remains blocked only on Tier-B operator-captured production evidence.
...
- Latest verified pushed GitHub baseline, `998ac09`, passed CI run `28344470006`: Lint (ruff), Postgres integration, Unit + drift checks, and G0 preregistration gate replay completed successfully.
- Terminal hard-stop and evidence-security cleanup is closed on `998ac09`: `ShadowWorkspaceService` rejects post-hard-stop ticks without trace/cycle growth until a fresh service window starts, hosted provider checks reject URL userinfo, invalid ports, private/reserved DNS targets, and inline `api_key` material, and evidence redaction scans structured JSON secret-key scalar fields while allowing non-secret proof metadata objects.
...
- `eval/g0/reports/report.{json,md}` has been regenerated from source commit `998ac09` with the committed sanitized controller-telemetry fixture; it reports `72/72` metrics measured and `gate_ready=True` without claiming production power/cost evidence.
- Production evidence custody hardening was closed in source at `6c45c56` and re-verified on `998ac09`: production render/capture now requires absolute external output and capture paths, production output rendering validates C2PA tool paths, hosted dashboard evidence requires retained `ops-dashboard-bundle.json` production operations proof, `production-evidence-verify` rescans retained metadata files for secret-shaped material, hosted evidence rejects inline secrets and private DNS targets, and sensitive evidence CID scoping prevents live unscoped sensitive rows from suppressing user-scoped sensitive writes while preserving tombstone replay blocking.
...
- Earlier pushed GitHub CI baseline `28343339309` passed on `8b03d27`; it is superseded by current run `28344470006` on `998ac09`.
