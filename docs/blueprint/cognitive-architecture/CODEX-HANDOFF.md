...
- Current verified source-bearing GitHub baseline, `9052600`, passed CI run `28395794365`: Lint (ruff), Postgres integration, Unit + drift checks, and G0 preregistration gate replay completed successfully.
...
- Production evidence custody hardening is closed through `9052600`: production render/capture now requires absolute external output and capture paths, full production capture requires an explicit external output root, production output rendering validates non-symlinked canonical C2PA tool paths, hosted dashboard evidence requires retained `ops-dashboard-bundle.json` production operations proof, `production-evidence-verify` rescans retained metadata files for secret-shaped material, hosted evidence rejects inline secrets and unsafe fetch targets, offline custody docs require an out-of-band retained bundle fingerprint, `summary.json.offline_verify` is validated only as a replay-command template and cannot embed its own expected fingerprint, production input artifacts are retained under `input-artifacts/`, executable verifier references such as the C2PA tool and provider-manifest commands are retained under `tool-artifacts/` and hash-bound in `preflight.json.executable_tool_references`, offline verification checks retained snapshot bytes rather than mutable external paths, renderer readiness output lists required production input artifacts with check/command/option ownership before environment values exist, `provider-manifest.production.template.json` gives operators the shared provider-check artifact for B1/B2/B4/B6/B7/B9/B10, retained `preflight.json` is included in the offline custody surface, and sensitive evidence CID scoping prevents live unscoped sensitive rows from suppressing user-scoped sensitive writes while preserving tombstone replay blocking.
- Production evidence review routing is now carried into the top-level handoff summary: `summary.json.parity_row_readiness` mirrors `preflight.json.parity_row_readiness`, `summary.json.row_review_source` points reviewers back to the retained preflight source of authority, and `production-evidence-verify` rejects summary/preflight row-readiness drift. This is routing metadata only; it does not create production evidence or flip strict-parity rows without a real operator-captured production bundle.
