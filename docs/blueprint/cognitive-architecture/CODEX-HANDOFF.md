...
2. Use the sanctioned production path only: create an external Tier-B custody packet with `infra/scripts/prepare-production-evidence-custody.py`, fill that packet's `production-render.env`, copy/fill `infra/templates/provider-manifest.production.template.json` into the external `$MNEMOSYNE_PROD_EVIDENCE_DIR/provider-manifest.production.json`, populate every other manifest-referenced production input artifact listed in `infra/templates/production-input-artifacts.checklist.md` under the same absolute external input-artifact directory, render the external production manifest with `infra/scripts/render-production-soak-manifest.sh --env-file <packet>/production-render.env --runtime-env-file <external-runtime-env>`, run `infra/scripts/capture-production-evidence.sh --env-file <external-runtime-env> --fingerprint-record-output <external-fingerprint-record-json>`, require strict `release-audit`, then run offline `production-evidence-verify` with `--fingerprint-record <external-fingerprint-record-json>` and `--report-output` set to a new external reviewer artifact path outside the bundle under review. Treat `summary.json.offline_verify` as verifier metadata, not as the fingerprint authority; `--expected-bundle-fingerprint` is a manual fallback only when populated from that out-of-band record, and `--internal-consistency-only` is diagnostic-only.
