...
- [ ] **G4 — Always‑on workspace controller.** The Phase 7 heartbeat safety floor is gate‑proven with bounded compute and rumination `0.0`; operational no-toggle retirement is gate-proven; promoted quality-per-unit-compute still requires reviewed controller telemetry.
