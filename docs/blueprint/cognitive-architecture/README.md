...
Documentation & spec **complete**; **G0 is implemented** in the repo-root `eval/g0/` harness (`runner.py`, `gate.py`, `baselines/baseline-0.json`, and `reports/report.json`). **G1 is next/in progress**: the implemented reliability-core slices wire multi-signal write priority with trust-bounded importance-sampling debias, prediction-error-gated consolidation metadata, evidence and projection-level reality-monitoring tags into abstention, legacy/unclassified projections to unknown-only abstention, retrieval-strengthening into evidence lifecycle metadata, schema-fast-path retrieval, and `contested` status for uncorroborated-but-congruent projections. The metacognition G0 metrics are backed by a runtime shadow trace monitor that scores confidence/outcome discrimination and abstention alignment. **G2/G3 foundations are now wired into a measured shadow path**: `mnemosyne.providers` exposes a typed `SpecialistModuleRegistry` with role/budget/critical-path contracts, `mnemosyne.cli specialist-manifest` exposes those contracts to operators, `mnemosyne.workspace.ShadowWorkspaceController` applies the bounded cycle, proto-self, and workspace bottleneck before recruiting specialists, and `mnemosyne.dreamer.SandboxedDreamer` provides tenant-scoped, CID-backed, low-trust replay candidates with `critical_path_allowed=false`. Gate evidence: `eval/g0/preregistrations/g1-reliability-core-schema-fast-path.json`, `eval/g0/preregistrations/g1-write-priority-debias.json`, `eval/g0/preregistrations/g1-projection-reality-monitoring.json`, and `eval/g0/preregistrations/g3-dreamer-shadow-ablation.json` pass against `eval/g0/baselines/baseline-0.json`; the G3 shadow replay slice improves `dreamer_shadow_corroborated_candidate_yield` from `0.0` to `1.0` while preserving `dreamer_shadow_contract`, ECE, abstention, confabulation, poison-block, and fast-path latency guardrails. Decisions are appended in `eval/g0/decision-log.jsonl`. Continue G1/G2/G3 by broadening reliability-core production evidence and keeping generative replay shadow-only until a future preregistered promotion gate explicitly proves target-up / guardrail-not-down behavior.
