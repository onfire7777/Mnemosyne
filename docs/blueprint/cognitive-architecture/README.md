...
Documentation & spec **complete**; **G0 is implemented** in the repo-root `eval/g0/` harness (`runner.py`, `gate.py`, `baselines/baseline-0.json`, and `reports/report.json`). **G1 — reliability core has begun**: the first reliability-core slice wires multi-signal write priority with trust-bounded importance-sampling debias, prediction-error-gated consolidation metadata, reality-monitoring provenance tags into abstention, retrieval-strengthening into evidence lifecycle metadata, schema-fast-path retrieval, and `contested` status for uncorroborated-but-congruent projections. Gate evidence: `eval/g0/preregistrations/g1-reliability-core-schema-fast-path.json` and `eval/g0/preregistrations/g1-write-priority-debias.json` passed against `eval/g0/baselines/baseline-0.json` with `multi_hop_ndcg_at_k` delta `+0.4065` and no ECE, abstention, confabulation, poison-block, or fast-path latency regression; decisions are appended in `eval/g0/decision-log.jsonl`. Continue G1 by broadening reliability-core coverage with real production adapters/evidence while preserving the same target-up / guardrail-not-down rule.
