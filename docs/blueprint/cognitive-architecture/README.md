...
**Location:** `docs/blueprint/cognitive-architecture/` · **Status:** G0 implemented; G1 next/in progress · **Owner:** onfire7777 · **Updated:** 2026‑06‑26
...
Documentation & spec **complete**; **G0 is implemented** in the repo-root `eval/g0/` harness (`runner.py`, `gate.py`, `baselines/baseline-0.json`, and `reports/report.json`). **G1 is next/in progress**: the implemented reliability-core slices wire multi-signal write priority with trust-bounded importance-sampling debias, prediction-error-gated consolidation metadata, evidence and projection-level reality-monitoring tags into abstention, legacy/unclassified projections to unknown-only abstention, retrieval-strengthening into evidence lifecycle metadata, schema-fast-path retrieval, and `contested` status for uncorroborated-but-congruent projections. The metacognition G0 metrics are now backed by a runtime shadow trace monitor that scores confidence/outcome discrimination and abstention alignment. **G2/G3 foundations are started**: `mnemosyne.providers` exposes a typed `SpecialistModuleRegistry` with role/budget/critical-path contracts, and `mnemosyne.dreamer` provides a shadow-only sandboxed replay specialist whose candidates are low-trust and promotion-gated. Gate evidence: `eval/g0/preregistrations/g1-reliability-core-schema-fast-path.json`, `eval/g0/preregistrations/g1-write-priority-debias.json`, and `eval/g0/preregistrations/g1-projection-reality-monitoring.json` passed against `eval/g0/baselines/baseline-0.json`; the projection-reality slice improves `projection_reality_abstention_recall` from `0.0` to `1.0` with no ECE, abstention, confabulation, poison-block, or fast-path latency regression. Decisions are appended in `eval/g0/decision-log.jsonl`. Continue G1/G2/G3 by broadening reliability-core production evidence, wiring typed specialists into the workspace controller, and keeping generative replay shadow-only until a preregistered target-up / guardrail-not-down promotion gate passes.
