...
| 2 | `phase2-sqlite-engine.md` (C2 + A1 cache + chaos harness + drift/ops-check/L4 suites) | to write | Phase 1 complete on branch `phase1/native-kernels` (merge pending) (ENGINE-CONTRACT, fixture parametrization, journal, registry from Phase 0; packed-BLOB dense seam owns the end-to-end 10× gate); inherits lane-invariant predicates deferred from Phase 0 |
