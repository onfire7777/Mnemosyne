...
**BINDING scale requirement (spec §4.2 parity — from Task-4 review):** Task 4 shipped scans that hydrate a full scoped LocalMemoryEngine per call (O(tenant-rows)) for byte-parity-now. Task 7 MUST push the candidate filter (`_candidate_hits` surface: tenant/branch/valid-window/trust/sensitivity/quarantine) into the SQL `WHERE` so the retrieval path is O(candidates), not O(tenant-rows) — replacing per-call full-tenant hydration on the `vector_search`/`lexical_search`/`graph_ppr` paths that `retrieve()` drives. Byte-parity vs the Local oracle MUST be preserved (the pushed-down SQL predicate must select exactly the rows `_candidate_hits` would, in the same order — prove with an oracle test that compares candidate SETS, not just final ranked output). **DATETIME HAZARD (from T5 review, binding):** `dt_to_json` emits variable-width fractional seconds, so lexical TEXT comparison of timestamps in SQL ≠ chronological order within a wall-clock second — any valid-window predicate pushed into SQL MUST NOT compare `dt_to_json` TEXT lexically; do the bitemporal window filter in Python via `models.parse_dt` (as T5's `as_of` fix did, `d810a61`) or emit a fixed-width comparison serialization. Include a mixed-precision oracle test. The end-to-end `retrieve()` on a seeded 10k-row tenant must be measured (add to Task 13 benches) and must not regress the §22.5 fast-path budget. If full predicate-pushdown parity proves intractable in T7, that is a BLOCKED escalation to the controller (spec parity is the goal), not a silent deferral.

**Interfaces:** Implement the `RetrievalPipelineOps` members for SqliteEngine (Task 1's Protocol): channel searches from Task 4 (refactored to SQL-predicate pushdown per the binding requirement above); port Local's `_apply_standing_scores`, `_confidence`, `_prediction_set_size`, `_reality_monitoring_report`, `_calibration_explain`, `_mark_retrieved_text_as_data`, `_merge_schema_fast_path_reports` usage — SHARED WHERE POSSIBLE: any helper that is pure (no store access) must be imported from its existing module, not copied (audit each; the spec forbids a third copy of pipeline logic). `_record_retrieval_access` = synchronous write matching shipped engines (R7's collision note: shipped behavior wins for parity; the spec's async-telemetry optimization is deferred, documented in the engine docstring + ledger). Consolidation duck-type contract (R3): get_evidence, export_tenant, append_evidence, add_relation, update_evidence_metadata, set_evidence_embedding, register_entity, upsert_assertion, retrieve, `adapters` attribute — a ConsolidationWorker smoke test flips passes from 'skipped' to 'complete'.
