...
| P5 | Production admission rails remain unfinished | MCP rotation R1c now has fixture-proven post-activation rollback, committed reproof/finalization for `activated`/`committed_recovered`, producer-side at-least-once keyed completion receipts, and whole-invocation cooperative rotator locking. Current working-tree evidence is green at 369/41/39/7/2 plus a hardware-admitted configured suite of 2,636 total (2,496 passed, 140 skipped, 0 failed/error). The evidence commit, exact-head CI, R2 cross-workflow locking, R3/R4, live rotation/no-op proof, Vault/trust/renewal operations, and restart-stability evidence remain open. | High |
...
| P9 | Knowledge freshness has one bounded post-commit closure step | Pre-completion Graphify/CBM snapshots are historical, and a ready local CBM index is not proof of freshness for the dirty working tree. Because Graphify, CBM, and gbrain are mutable external indexes, their final refresh must run after the evidence commit and be recorded against that exact head in PR #12; any later source edit reopens the check. | Medium, controlled |
...
- **A2. Run the queues under their real gates:** retain R1c's now-green working-tree targeted/full-suite evidence, land it with exact-head CI, then obtain a new strong admission for candidate-v19's remaining immutable external manifest/runtime custody and the 24/24 `qa_scale_dev_v1` receipt; only then may v19 consume its protected attempt.
- **A3. Hygiene:** land the clean stacked PR #12 only after independent review, explicit residual boundaries, an evidence commit, and green exact-head CI; refresh Graphify/CBM/gbrain only against that commit and keep one canonical work stream per machine during eval windows.
