...
- Current safe coding concurrency is **zero new implementation writers**:
  ...
  receipt-level lifecycle update, now `MERGED` as PR #92; no *new* writer is
  admitted at this baseline, so apart from `T4`'s own branch-resident receipt
  maintenance — this recomputation and its pinning test, both inside `T4`'s
  declared lease — only read-only reviews may run.
