...
**Date:** 2026-06-21 · **Status:** resolved (7/7, all high confidence; implementation status refreshed 2026-06-29)
**Method:** 8-agent read-only analysis fleet, grounded in the blueprint's standing decisions + the
then-current `src/mnemosyne` tree. The decision record below is retained as the
ADR source; implementation notes in parentheses have been refreshed as source
wirings landed.

## Decisions

- **OQ1 — PPR latency.** Cached PPR on the **fast** path; live PPR only in **async deep** mode behind the engine adapter; swap a graph specialist only on a profiled SLO tripwire (~1e7–1e9 edges/tenant). Consolidation precomputes a per-relation PPR score the fast path reads under a small cap. *(2026-06-29: default-off Postgres cached-PPR seam exists; local bench still reports live recompute timing and does not fabricate cached latency.)* → **FR-11, FR-3 P95**.
...
- **OQ7 — Capability overhead.** Asymmetric: **full mediation on writes/tool-flows, trust-tier-only on reads** + a cheap data-not-instruction envelope (avoids CaMeL's ~2.7× read-path tax). Wire `sanitize_retrieved_text`. *(2026-06-29: local and Postgres read return paths attach `retrieved_text` data-not-instruction metadata; the bench now measures primitive overhead rather than an absent call site.)* → **FR-7, I11**.
