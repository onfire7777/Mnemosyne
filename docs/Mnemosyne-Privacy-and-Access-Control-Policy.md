...
**Detector status (stated honestly).** The shipped classifier `classify_privacy` detects email, phone, SSN, Luhn-valid payment-card numbers, valid IPv4 addresses, DOB/date-of-birth labels, passport IDs, and street-address shapes, and ingest escalation is `sensitivity = max(request.sensitivity, detector)` (`ingestion.py`). Operators handling regulated deployments still MUST set ingest-time sensitivity floors for source classes that exceed those built-in detector shapes, and MUST run `privacy-backfill-report` plus an audited `privacy-backfill-apply --confirm-apply` pass before relying on legacy default-`0` rows.
...
A pre-existing item at `sensitivity 0` is *unclassified*, not *certified public*. Any migration that introduces this policy onto an existing store MUST run a backfill detection pass (or quarantine-to-S2 the un-reviewed personal-data tables) before relying on ceilings. `mnemosyne privacy-backfill-report --tenant <tenant>` is the read-only local/Postgres preflight for this: it emits CIDs, content hashes, detected PII tags, and recommended sensitivity/access-policy floors without raw content. `mnemosyne privacy-backfill-apply --tenant <tenant> --confirm-apply` applies the same detector output through the audited engine contract: it raises sensitivity, sets `access_policy.data_class=pii`, raises `access_policy.max_sensitivity`, refreshes privacy metadata, and moves the row's vector partition out of public search. Until backfill completes, treat the store as containing un-flagged S2 and lower the `agent` ceiling accordingly. Log the backfill coverage; silent reliance on default-`0` over legacy rows is the one way this policy fails open.
...
- The ingest privacy detector now recognizes email, phone, SSN, Luhn-valid payment-card numbers, valid IPv4 addresses, DOB/date-of-birth labels, passport IDs, and street-address shapes (§2.1). `privacy-backfill-report` and `privacy-backfill-apply --confirm-apply` provide the read-only and audited mutation paths for legacy local/Postgres rows. Regulated deployments still need operator-owned precision/recall review before relying on detector coverage for jurisdiction-specific data classes.
...
2. **Legacy-row backfill operations** must run `privacy-backfill-report` and the explicit `privacy-backfill-apply --confirm-apply` mutation pass for built-in detector hits, then classify or quarantine any jurisdiction-specific regulated classes that exceed the local detector.
