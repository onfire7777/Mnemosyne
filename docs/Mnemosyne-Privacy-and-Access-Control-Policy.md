...
2. **Legacy-row backfill** must classify or quarantine historical default-`0` rows identified by `privacy-backfill-report`, plus any jurisdiction-specific regulated classes that exceed the built-in local detector.
