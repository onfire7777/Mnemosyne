...
| `M04-UNRESOLVED-CAL` | Agreement between observed multiplicity/abstention and gold unresolved state. | **`= 1.0`** for the deterministic Stage A matrix and included in aggregate `passed`. Numeric ECE is reported only when the SUT supplies confidence; otherwise `unsupported` (M10 precedent). |
