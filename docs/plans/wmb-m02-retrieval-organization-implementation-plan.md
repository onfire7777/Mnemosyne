...
cited anywhere below. Every factual claim in §3 and §4 is grounded in a
targeted read of the named file at the verified base above, and each such claim
names its exact file and line anchor so a reviewer can re-derive it without
...
carve-out has therefore **lapsed**, and the map must be recomputed from
`main@effc5e03` before it is treated as operative. This is recorded, not worked
around: no lease in §11 may be taken up until G1 discharges it. Neither
`GOAL.md`, `.planning/STATE.md`, nor the lease map is edited by this lane.

### 2.3 Technical prerequisites — verified present at this base

| # | Prerequisite | Anchor | State |
|---|---|---|---|
| T1 | Closed common ABI (`wmbs/0.1-draft`) with schema, canonical JSON, `validate_definition`, `ProtocolValidator` | `eval/public/schema/wmbs-0.1-draft.schema.json`; `eval/public/adapters/whole_memory_reference.py:21-33` | Present; SAFE/BOUNDED for contract validation only |
...
**Q3 — `_fractional_result`'s top-level `interval` is insertion-order
dependent.** `eval/public/scoring.py:465-478` sets
`interval = next(iter(intervals.values()))` — for `longmemeval-retrieval-v1`
that is `recall_at_5`, for `hipporag-retrieval-v1` it is `recall_at_2`. The
runner gates on that single value (`runner.py:438-446`), so the top-level
`interval` is silently metric-ambiguous. *M02 consequence:* M02 emits the
M01-shaped `interval: {"method": "descriptive"}` (carrying a method and no
metric value) plus keyed `intervals`, and never reads the ambiguous field. No
edit to shared code; the constraint is discharged inside M02's own scorer.
...
**Q11 — `tests/test_parity_retrieval.py` is not retrieval-quality proof.** It
derives its gold from the engine's own retrieval output, so it is a
self-consistency/parity check, not an independent quality measurement.
*M02 consequence:* not citable as an M02 baseline or quality reference. M02's
gold is generator-owned and derived before any retrieval runs (§7).

**Q12 — MemoryAgentBench averages caller-supplied scores.**
`eval/public/adapters/memoryagentbench.py` aggregates scores handed to it; it
is not upstream behavioural evidence. *M02 consequence:* not citable as
official upstream M02 evidence; the official-adapter row stays `DEFERRED`
(§10).
...
| `ingest` / `retrieve` / `answer` operations | `_OPERATIONS` at `whole_memory_reference.py:33` | The three universal operations M02 exercises |
...
`interval: {"method": "descriptive"}`, `metrics`, `passed`, `profile`,
`profile_version`, `total`, `trace_count`, plus keyed `intervals` — and it
never reads the ambiguous top-level interval value (Q3).
...
| RED-A7 | `test_metrics_shape_is_descriptive_and_finite_corpus_only` | Output carries `finite_corpus_only: True`, `interval: {"method": "descriptive"}`, keyed `intervals`, and no metric value in the top-level interval (Q3) |
...
- **G1** — the lease map must be recomputed from `main@effc5e03`. Its recorded
  baseline is 22 commits stale and `GOAL.md`'s own lapse detector fails at this
  base (§2.2). Until then the map is not operative, and its `U-MODULES` row
  still reads `No lease`.
...
Stage B additionally requires **G3**; Stage C additionally requires **G4**.

These four are the only irreducible blockers. Every other gap named in this
document is either deferred with its exact missing evidence (§13) or
quarantined with its smallest RED regression and owning lane (§3.2).
...
- No edit to any quarantined asset, to PR #94 paths, to `GOAL.md`,
  ...
  doc. This lane wrote exactly one new file.
