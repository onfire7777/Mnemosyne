...
def write_bundle(destination: Path, *, benchmark: dict[str, Any], metadata: dict[str, Any], metrics: dict[str, Any], traces: list[dict[str, Any]]) -> None:
    ...
    try:
        ...
        _write_json(temp / "config.json", {"family": metadata["family"], "interval_method": metadata["interval_method"], "scoring_profile": metadata.get("scoring_profile", "smoke-hit-at-k-v1"), "suite": metadata["suite"]})
...
def verify_bundle(bundle: Path | str) -> dict[str, Any]:
    ...
    family, method, profile = config.get("family"), config.get("interval_method"), config.get("scoring_profile")
    allowed_profile = {
    ...
    if profile == "smoke-hit-at-k-v1":
        ...
        try:
            expected_metrics = score_profile(profile, _scoring_labels(benchmark.get("data")), traces)
        except ScoringError as exc:
            raise BundleError("generalized scoring profile recomputation failed") from exc
...
def write_report(
    ...
    source_result, reproduced_result = verify_bundle(source_root), verify_bundle(reproduced_root)
    ...
    report = {
        "bindings": {
            "reproduced_bundle_manifest_sha256": _digest(reproduced_root / "bundle-manifest.json"),
            "source_bundle_manifest_sha256": _digest(source_root / "bundle-manifest.json"),
            **{f"{name.replace('.', '_')}_sha256": _digest(source_root / name) for name in ("benchmark.json", "metrics.json", "traces.jsonl")},
        },
        "git_sha": git_sha,
        "generated_at": generated_at,
        "evidence": evidence,
        "command": [
            "mneme", "eval-public", "--write-report", str(source_root),
            "--reproduced-bundle", str(reproduced_root),
            "--report-output", str(output.resolve()),
            "--report-note", str(note.resolve()),
        ],
        "publication": {
    ...
    try:
        ...
        raise
    return {"report": str(output.resolve()), "report_note": str(note.resolve()), "sha256": report_digest, "valid": True}
...
def verify_report(report_path: Path | str, report_note: Path | str) -> dict[str, Any]:
    ...
    if set(report) != {"bindings", "command", "evidence", "generated_at", "git_sha", "publication", "reproduced_bundle", "source_bundle", "suite", "version"} or report["version"] != 1:
        raise BundleError("invalid report schema")
    if len(report.get("git_sha", "")) != 40 or set(report["git_sha"]) - set("0123456789abcdef"):
        raise BundleError("invalid report git SHA")
    source, reproduced = Path(report["source_bundle"]), Path(report["reproduced_bundle"])
    ...
    expected = {
        "reproduced_bundle_manifest_sha256": _digest(reproduced / "bundle-manifest.json"),
        "source_bundle_manifest_sha256": _digest(source / "bundle-manifest.json"),
        **{f"{name.replace('.', '_')}_sha256": _digest(source / name) for name in ("benchmark.json", "metrics.json", "traces.jsonl")},
    }
    ...
    expected_command = [
        "mneme", "eval-public", "--write-report", str(source.resolve()),
        "--reproduced-bundle", str(reproduced.resolve()),
        "--report-output", str(report_file.resolve()),
        "--report-note", str(note_file.resolve()),
    ]
    if report.get("command") != expected_command:
        raise BundleError("report command provenance mismatch")
    if report.get("evidence") != _report_evidence(source.resolve(), reproduced.resolve()):
    ...
    if any(report.get("publication", {}).get(flag) is not False for flag in ("publishable", "pbpp_headline_eligible", "independent_external_reproduction")):
    ...
    for value in (_digest(report_file), expected["source_bundle_manifest_sha256"], expected["reproduced_bundle_manifest_sha256"], report["git_sha"]):
    ...
    for projection in (
        ...
        json.dumps(report["evidence"]["metrics"], sort_keys=True, separators=(",", ":")),
        json.dumps(
            {
    ...
    return {"report": str(report_file.resolve()), "report_note": str(note_file.resolve()), "sha256": _digest(report_file), "valid": True}
...
def _report_evidence(source: Path, reproduced: Path) -> dict[str, Any]:
    ...
    eligible = metadata.get("eligible_count", len(questions) if isinstance(questions, list) else trace_count)
...
def _scoring_labels(benchmark: Any) -> list[dict[str, Any]]:
    if not isinstance(benchmark, dict) or not isinstance(benchmark.get("questions"), list):
    ...
    for question in benchmark["questions"]:
        ...
        gold = question.get("gold_references", question.get("gold_doc_ids", question.get("gold_passage_ids")))
        if gold is not None:
